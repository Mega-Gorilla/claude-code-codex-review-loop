...
合意済みbaselineはClaude Code coder / Codex reviewerです。両roleへClaude Code / Codexを独立設定し、同一provider同士を含む4組み合わせへ拡張するD-032は**Decided（2026-09-15）・native adapter未実装でIssue #52が追跡します**（[implementation plan](../plans/implementation-plan.md)のD-032拡張）。以下はroleで責務を表します。Phase 8までの実行基盤の完成と、両providerの実接続完了は区別します。
