...
- **integrityの検出時安全**: C-06の整合性検出は、`MERGING`ではmerge実行後・成否不明ならoutcome照会を維持して確定を最優先し（integrity対応は確定後に再入力）、preconditions段階では承認失効付きで安全停止する。activeな6 stateではhalt gate（process停止・checkpoint完了event）を経て初めて`BLOCKED`へ入り、実行中のagent processを残さない
