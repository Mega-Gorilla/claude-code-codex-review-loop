...
**遷移registry**: states、events、commands、guard条件、terminal / resumable分類を**単一のregistry**として定義し、完全遷移表と遷移図をregistryから導出・照合する。guardは自由なpredicateではなく**有限のtyped discriminator**（`awaiting`値、`pending_record`一致、`progress`値、`block`のkind / reason、`cancelling` / `return_to` / `recovery_to` / `deferred_integrity` / `incident_target` / `incident_audit`の有無とbinding一致、および集合差分からC-01が決定論的に導出する`IncidentCoverage`の2値）に限定し、共通規則（cancel / failure等）もregistry内で個別ruleへ展開して、**到達可能なMachineState付随値の各組合せ × 各eventに一致するruleは0件または1件**とする（overlapはdiscriminator全値の展開で機械的に検査しfail。優先順位による解決はしない）。target experienceの「State model」節の図はユーザー向け簡略図として維持し、計画文書はnormativeな期待挙動、実装のcode registryが実行可能な単一の正本であり、表・図はregistryから生成してsnapshot照合する。簡略図が省略している遷移（失敗系からのresume、各状態でのcancel可否、GitHub投稿・確認失敗、preflight失敗）もregistryでは全て定義する。

**MachineState**: 可視の17 stateとは別に、immutableな`MachineState`が付随情報を保持する: `return_to`（tool permissionからの復帰先）、`recovery_to`（`EV_RUN_FAILED`で入った`FAILED`の安全な再開地点）、`block`（`BLOCKED`の停止理由と解消policy。kindは`PROGRESS`（本来の継続 = state・command列・awaitingと、reason / budget / counter snapshot / fingerprint）/ `EXTERNAL_DEPENDENCY`（hostが報告し永続化確認済みの外部依存。継続あり）/ `RECORD_INTEGRITY`（C-06が検出したcanonical commentの改変・削除・sequence gap。継続なし。evidenceは参照可能なrecordが存在しない違反も表せる`IntegrityEvidenceRef`）の3種。block attemptのbindingは進入eventのevidence bindingを再利用する。`recovery_to`とは排他で、block進入時は旧`recovery_to` / `return_to` / 旧`block`を破棄する）、`cancelling`（進行中のcancel attempt。bindingは`USER_CANCEL` recordのbindingを再利用する。保持中はbinding一致の停止完了event・失敗event・integrity検出（deferred記録のみ）以外の全semantic eventを拒否する）、`deferred_integrity`（`MERGING`のoutcome段階・`cancelling`中・incident記録段階に検出済みintegrity violationを**集合として**typed保持。手続き中（outcome照会 / cancelling / incident記録 / halt gate）の別binding検出は排他guard付き検出ruleが常にunionで集合へ追加し、検証済みrecordの記録済み集合との差分は有限discriminator（`IncidentCoverage`）としてC-01が導出、`COMPLETE`まではterminalへ進まない — 未記録残余は次のincident cycleへ直列化され、silentに破棄されない）、`incident_target`（incident記録完了後に進むterminal。記録の3段階（作成前 / 永続化待ち / 検証後）を通じて保持され、全deferred記録後のterminal遷移でのみ消費される）、`incident_audit`（incident切替時に破棄したstale pendingの監査参照。terminalまで保持され、incident作成commandのpayloadへ常に含まれる）、`awaiting`（発行済みcommandに対応する「次に受理してよい応答」の有限typed discriminator。応答eventはguard一致でのみ受理・消費され、順序を飛ばした応答・消費済み応答の再入力は構造化errorで拒否される）、`pending_record`（永続化の確認待ちrecordのkindとopaque binding）。典型例は`AWAITING_TOOL_PERMISSION`で、`RUNNING_REVIEW`と`APPLYING_FIXES`のどちらから入ったかを`return_to`が保持し、permission承認eventで元の処理へ戻る。**復帰先・再開地点をeventの引数として外部から渡す設計は採らない**（不正な状態jumpを防ぐ）。
...
- **一律fresh reviewは採らない**。`EV_RUN_FAILED`で入る`FAILED`は`recovery_to`（進入元state。`pending_record` / `awaiting` / `cancelling`は変更せず引き継ぐ）を、`BLOCKED`へ入るruleは`block`を設定する（両者は排他）。head変更・checkpoint不整合等で安全を証明できない場合のみfresh reviewへfallbackする
- **cancellingはstate分類より優先する横断resume規則を持つ**: `cancelling`保持中は、terminal / `MERGING`を除く全stateで、明示resumeが`CMD_HALT_RUN(binding)`の再発行だけを返す（停止・checkpoint完了が他のあらゆる再開に先行する。resumable state滞在のままでも同じ）
- **`FAILED`のresumeは優先順位で決める**: (1) `pending_record`があれば同一bindingの冪等な永続化確認のみを再開（次agentを起動しない）、(2) `awaiting`があれば対応するcommandの再発行・結果照会、(3) `recovery_to`があればその駆動command（値ごとに全数定義）を発行する。**preflight NGで作られた付随値のない`FAILED`はresume系eventを構造化errorで拒否し、新しいrunの`initialize`（preflight再実行）だけを復帰経路とする**。resumable state滞在中の失敗は既存の付随値を上書きしない
- **`BLOCKED`のresumeは停止理由の解消gateを通す**: 同一条件での単純resumeはkindに関わらず継続を再現せず`BLOCKED`を維持し、commandを発行しない。解消evidenceは**record自身のbindingと解除対象の`target_block_binding`を別fieldで持ち**（対象参照はcanonical本文 / metadataからC-06が検証・抽出）、blockとの完全一致（target binding / reason / budget / counter snapshot / fingerprint / head）で受理される。limit引き上げ検証（`LIMIT_REACHED`）またはuser-input record `BLOCK_INTERVENTION`（2経路）のcanonical検証（`NO_PROGRESS` / `EXTERNAL_DEPENDENCY`）だけが、保存された本来の継続を1回再現する。`PROGRESS` / `EXTERNAL_DEPENDENCY`のgeneric fallback（head変更等）は継続を破棄しfresh reviewへ入る。**`RECORD_INTEGRITY`はgeneric fallbackを受理せず**、整合性復元の再検証またはsalvageによる新baseline確立を表す専用evidenceのみを出口とする（fail closed。salvage手順の提供はPhase 14）
- retry可能な失敗とterminalな失敗をevent / guardで区別する
- **integrityの検出時安全**: C-06の整合性検出（violation binding付き。同一違反の再検出は冪等）は、**受理する全経路で既存承認を即時かつ冪等に失効**させる。`MERGING`のmerge実行後・成否不明では`deferred_integrity`として保持したままoutcome照会を維持して確定を最優先し、確定ruleが必ず消費する — merge完了なら検証済みincident record（`INTEGRITY_INCIDENT`。canonical record gateを通し、投稿失敗時は非terminalのまま冪等に再実行できる）を確認後に`MERGED`へ、未実行確認なら`RECORD_INTEGRITY` blockへ（通常resumeでのgate迂回を防ぐ）、**成否不明はintegrityで上書きせず`MERGING`のまま照会のみを継続**（integrityの復元 / salvageだけではagent / merge commandを発行できない）、cancel系は検証済みincident record確認後に`CANCELLED`へ進む（terminal後のevent再入力に依存しない）。`MERGING`の明示resumeは`MERGE_OUTCOME`（origin維持）で照会のみ、preconditions待ちで再検証のみを再発行し、incident記録の各段階の失敗・resumeは対応command（作成依頼または冪等な永続化）だけを再発行する。横断規則（cancelling / halt gate / incident処理中）と通常のfailure / resume ruleは「手続き中 / 手続き中でない」の**明示の有限guard**で排他になる（優先順位では解決しない）。cancel完了からincident記録へ切り替える際はstale pendingを破棄し、監査参照をincident作成へ引き継ぐ。preconditions段階では承認失効付きで安全停止する。activeな6 stateではhalt gate（process停止・checkpoint完了event）を経て初めて`BLOCKED`へ入り、実行中のagent processを残さない
- **cancelは2系統・停止完了gate**: いずれの経路でも、`CANCELLED`へはactive processの停止とcheckpoint保存の完了event（`EV_CANCELLATION_COMPLETED`）を受けて初めて遷移する（intent検証は真正性の保証であり、process停止の保証ではない）。通常の対話cancelはcancel intentのcanonical検証後、同一stateに留まり`cancelling`（attempt binding = `USER_CANCEL` recordのbindingを再利用）を設定して停止command（`CMD_HALT_RUN(binding)`）だけを発行し、**binding一致の完了event**を待つ（過去attemptの遅延・重複完了は拒否。cancel中はstale pendingのVERIFIEDを含む全semantic継続を拒否し、resumeでも停止完了が最優先）。Ctrl+C等の緊急停止はC-03 / C-08が停止・checkpointを行い、run / checkpointへのbindを検証した完了eventを直接入力する（`MERGING`中の割込みのみeventを入力せずcheckpointし、resumeで照会を再開する）。resumeは直前の安全なcheckpointから新しいrunとして開始する
- **bounded-progressの境界**: 上限・膠着の判定結果は、**同じbounded loopをもう1回継続する遷移だけ**が持つ`progress` discriminator（`CONTINUE` / `LIMIT_REACHED` / `NO_PROGRESS`）として入力する。budgetは`REVIEW_ROUND`（counterは新しいfix roundの開始で1回だけ増加し、同一roundの完了では二重計上しない）と`CLARIFICATION_TURN`（1 turn = 質問／再提出とCodex回答の一往復。**5回目のturn開始は許可し、6回目の開始を停止する**。REVISE_AND_RESUBMITは同一fingerprintのclarification turnとして**共通の5-turn counter**を消費する）の2つで、消費点・判定点はregistryのdataとして明示する。`CONTINUE`の場合のみcommandを発行し、上限到達時は`BLOCKED`へ遷移して（`blocked_continuation`を保存）agent commandを一切発行しない。**loopを終了する結果は上限turnで得られたものでも常に処理する**
...
| AC-C01-07 | `MERGING`中のcancelがmerge結果の照会を経由し、結果を確定できない場合に`CANCELLED`ではなく`MERGE_FAILED`へ遷移する（`deferred_integrity`保持中の成否不明は`MERGING`に留まり照会のみを継続し、outcome確定までagent / merge commandを発行しない） |
...
| AC-C01-11 | `BLOCKED`は停止理由（kind / binding / reason / budget / counter snapshot / fingerprint / head）を保持し、同一条件での単純resumeはcommandを発行せず`BLOCKED`を維持する。解消evidenceはrecord自身のbindingと解除対象の`target_block_binding`を分離して持ち、blockとの完全一致で受理される。limit引き上げ検証またはuser-input record `BLOCK_INTERVENTION`（2経路）のcanonical検証だけが、保存された本来のcommand列（付随actionを含む）を1回再現する。同一targetへの別record（初回のみ受理）・別blockを指すrecord・同一recordのreplayが区別して拒否される |
| AC-C01-12 | progress以外のblockがPhase 1 registryで定義される: C-06の整合性検出（AC-C06-06〜08）は受理する全経路で承認を即時・冪等に失効させ、`MERGING`のoutcome段階では`deferred_integrity`として保持したまま照会を維持して確定を最優先し（確定ruleが必ず消費し、検出済み違反をsilentに失わない）、preconditions段階では安全停止、activeな6 stateではhalt gateのprocess停止完了後、resumable stateでは直接、`RECORD_INTEGRITY` blockへ入る（進入時にpending / awaiting / recovery_to / return_to / 旧blockを破棄）。`RECORD_INTEGRITY`はgeneric fallbackを受理せず、整合性復元またはsalvage確立の専用evidence（target binding一致）のみを出口とし、未修復chainからfresh reviewを起動できない。terminal前のincident監査記録はcanonical record gateを通過した検証済みrecordを要求する。外部依存はhostの報告record検証で`EXTERNAL_DEPENDENCY` blockへ入り、intervention検証で継続を再現する。preflight NGの`FAILED`はresume系eventを拒否し、新しいrunの`initialize`だけを復帰経路とする |
