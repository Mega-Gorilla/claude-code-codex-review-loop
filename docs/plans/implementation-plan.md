...
- **integrityの検出時安全**: C-06の整合性検出（violation binding付き。同一違反の再検出は冪等）は、**受理する全経路で既存承認を即時かつ冪等に失効**させる。`MERGING`のmerge実行後・成否不明では`deferred_integrity`として保持したままoutcome照会を維持して確定を最優先し、確定ruleが必ず消費する — merge完了なら`MERGED`を偽らず監査記録・失効を伴って終了、未実行・不明なら`MERGE_FAILED`ではなく`RECORD_INTEGRITY` blockへ（通常resumeでのgate迂回を防ぐ）、cancel系は`CANCELLED`へ進めつつ監査記録を残す（terminal後のevent再入力に依存しない）。preconditions段階では承認失効付きで安全停止する。activeな6 stateではhalt gate（process停止・checkpoint完了event）を経て初めて`BLOCKED`へ入り、実行中のagent processを残さない
