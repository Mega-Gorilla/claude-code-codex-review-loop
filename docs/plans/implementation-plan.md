...
- **`BLOCKED`のresumeは停止理由の解消gateを通す**: 同一条件での単純resumeはkindに関わらず継続を再現せず`BLOCKED`を維持し、commandを発行しない。解消eventは**block attemptへの完全binding一致**（binding / reason / budget / counter snapshot / fingerprint / head）で受理され、limit設定の引き上げ検証（`LIMIT_REACHED`）またはuser-input record `BLOCK_INTERVENTION`（2経路）のcanonical検証（`NO_PROGRESS` / `EXTERNAL_DEPENDENCY`）だけが、保存された本来の継続を1回再現する。`RECORD_INTEGRITY`は解消eventを受理せず、継続を破棄するfallback（fresh review）だけを出口とする。head変更等のfallbackは全kindで継続を破棄しfresh reviewへ入る
...
| AC-C01-11 | `BLOCKED`は停止理由（kind / binding / reason / budget / counter snapshot / fingerprint / head）を保持し、同一条件での単純resumeはcommandを発行せず`BLOCKED`を維持する。解消eventはblock attemptへの完全binding一致で受理され、limit引き上げ検証またはuser-input record `BLOCK_INTERVENTION`（PowerShell転記 / GitHub直接commentの2経路）のcanonical検証だけが、保存された本来のcommand列（付随actionを含む）を1回再現する。過去または別block向けの解消event、消費済みblockへのreplayは拒否される |
| AC-C01-12 | progress以外のblockがPhase 1 registryで定義される: C-06の整合性検出（AC-C06-06〜08）は全非terminal stateから`RECORD_INTEGRITY` blockへ入り`pending_record` / `awaiting`を破棄し、解消eventを受理せずfallback（fresh review）のみを出口とする。外部依存はhostの報告record検証で`EXTERNAL_DEPENDENCY` blockへ入り、intervention検証で継続を再現する。preflight NGの`FAILED`はresume系eventを拒否し、新しいrunの`initialize`だけを復帰経路とする |
