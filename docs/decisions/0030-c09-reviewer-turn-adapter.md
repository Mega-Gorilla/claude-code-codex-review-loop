...
6. **reportからの対象head抽出は`ReportHeadPort`**（本実装はC-10のreport parser）、**advertised headの再観測は`AdvertisedHeadPort`**で、それまでは`UnavailableReportHead` / `UnavailableAdvertisedHead`が`PortUnavailableError`で停止する。`AdvertisedHeadPort`の本実装は`runtime/advertised_head.py`の`GitHubAdvertisedHead`（C-05の`get_pull_request`、ADR-0012）で、呼出ごとにGitHubを読み、取得値を加工せずhead SHAだけを返す。C-05の失敗分類は`advertised_head:<category>`（`transient` / `not_found` / `auth` / `permanent`）の固定stageへ写し、応答本文・URLを含めない。PRのstate（closed / merged）の扱いはC-10の責務である。timeoutや抽出失敗で対象headが無い場合は空文字を渡し、`reported_invalid`の`HeadMismatch`になる。`HeadMismatch`時の非投稿はC-10の責務で、本moduleは判定を返すだけである。
