...
9. argvは`<codex> exec --ephemeral --ignore-rules -C <checkout> [-o <last message file>] -`に固定し、promptはstdinで渡す（`-o`は起動facadeが指定する。決定15）。任意argv・`-c`上書き・`--add-dir`・`-p`の入口をAPIに持たせない。既存のargv choke point（P-006）へCodex固有の禁止語彙（approval / sandbox迂回、full access、`shell_environment_policy.inherit=all`、`sandbox_permissions`のfull read）を追加し、`-c`で到達できる既知の危険値も拒否する。
...
13. **第2段階（process facade PR）がpreflightを担い、次の2つを両方必須にする**。片方はもう片方の代替にならない。
    ...
    - **probeの同一性とpositive control**（PR #62で追加）: probeはfacadeが選ぶ本packageの`sandbox_probe.py`と現在のinterpreterで、fileの内容（改行をLFへ正規化）を固定digest `PROBE_DIGEST`と照合し、不一致なら起動しない。呼出側からprobe・接続先・argv prefixを受け取らない。実測の前に、sandboxの**外**で同じprobe・同じ接続先（`api.github.com:443`。TCP handshakeだけでrequestは送らない）を実行し、書込・読取・接続が通ることを確かめる。これが無いと、到達不能な接続先や壊れたpathでも`denied`に見えるfalse positiveを排除できない。controlにはprotected rootを渡さず、実repositoryへは一時fileも書かない。
